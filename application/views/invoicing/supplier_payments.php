...
<style>
    ...
    .payment_error{display: none;    margin: 0px;
    color: red;}
           ...
           // if($usertype == '5') { ?>
            ...
            <div class="col-md-12">
                ...
                <form method="post" id="saveform">
                    ...
                    <button type="submit" name="btn_payment" class="btn btn-lg btn-success" style="float: right;">Save Payment</button>
                <table class="table table-striped table-hover table-bordered1 stl_table" id="show_reporttb" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="hidden" name="payment_data[<?= $i; ?>][balance]" class="balance_amt" value="<?= $balance; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][report_id]" value="<?= $ReportDetail->report_id; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][user_id]" value="<?= $ReportDetail->user_id; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][inv_user_id]" value="<?= $ReportDetail->inv_user_id; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][inv_no]" value="<?= $inv_no; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][ref_no]" value="<?= $ref_no; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][cost_id]" value="<?= $ReportDetail->cost_id; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][amount_type]" value="<?= $ReportDetail->amount_type; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][category_id]" value="<?= $ReportDetail->category_id; ?>">
                                <input type="hidden" name="payment_data[<?= $i; ?>][subcategory_id]" value="<?= $ReportDetail->subcategory_id; ?>">


                                <input type="text" name="payment_data[<?= $i; ?>][rdata]" class="form-control tform rdata" placeholder="Payment Date">
                                <input type="number" name="payment_data[<?= $i; ?>][amount]" class="form-control tform recpt_amt" min="0" max="<?= $balance; ?>" placeholder="Payment Amount">
                                <p class="payment_error">Payment amount should be less than or equal to balance amount</p>
                            </td>
...
$(document).on('change','.recpt_amt',function(){
    $('.payment_error').hide();
    var this_val = $(this);
    this_val.closest('tr').find('.payment_error').hide();
    ...
    {
        this_val.closest('tr').find('.payment_error').show();
...
jQuery(document).on('click','.btn_fcreport',function(){
   ...
   jQuery('#saveform').on('submit', function(e){
      ...
      var src = '<?php echo BASE_URL; ?>'+'invoicing/savepayment';
