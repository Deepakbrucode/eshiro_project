...
class Reports extends CI_Controller
{
	...
	public function analysis_data()
	{
		...
		$ReportDetails = $this->reports_model->getDetails('report', array('user_id' => $final_client_id, 'status' => 1, 'cost_id !=' => '', 'month(report_date)' => $month, 'year(report_date)' => $year, 'amount_type' => 'expense'), '', array('custom_where' => '(report_type="1" or report_type = "7")'));
	...
	public function unanalysis_data()
	{
		...
		$data = array(
			'view_file' => 'unanalysis_data',
			'current_menu' => 'unanalysis_data',
			'cusotm_field' => 'report',
			'site_title' => 'eShiro',
			'logo'		=> 'logo',
			'title' => 'Report',
	...
	public function analysis_data_month()
	{
		...
		$ReportDetails_month = $this->reports_model->getDetails('report', array('user_id' => $final_client_id, 'status' => 1, 'amount_type' => 'expense', 'cost_id !=' => ''), 'YEAR(report_date) as year_val, MONTH(report_date) as month_val', array('orderby' => 'YEAR(report_date),MONTH(report_date)', 'disporder' => 'asc', 'groupby' => 'MONTH(report_date), YEAR(report_date)', 'custom_where' => '(report_type="1" or report_type = "7")'));
		// echo $this->db->last_query();
		// $CostDetails = $this->costcentre_model->getDetails(array('category_id !=' => '', 'subcategory_id !=' => '','user_id' => $final_client_id));
	...
	public function unanalysis_data_month()
	{
		...
		$data = array(
			'view_file' => 'unanalysis_data_month',
	...
	public function bank_statement()
	{
		...
		$bank_stat_yr = $this->reports_model->get_bankstatmt_year($current_userid, $end_month, $bank_id);
		// echo $this->db->last_query();
	...
	{
		$array_merge = [];
		$MonthDetails = $this->reports_model->bankstat_montdetails($month, $year_start, $filtered_client_id, $bank_id);
		$start_date = $year_start . "-" . $month . "-01";
		$CRDetails = $this->reports_model->total_cr($start_date, $filtered_client_id, $bank_id);
		$DRDetails = $this->reports_model->total_dr($start_date, $filtered_client_id, $bank_id);
	...
	public function ledger_creation($report_type = 1)
	{
		...
		//Arul->added the bank stement to leger
		//$bank_stat_yr = $this->reports_model->get_ledger_year($current_userid,$end_month,$report_type);
		$bank_stat_yr = $this->reports_model->get_ledger_year_new($current_userid, $end_month, $report_type);
		// echo "<pre>"; print_r($bank_stat_yr); die;
		if ($report_type == '1') {
...
// die;
	...
	public function ajax_ledger_bankjournal()
	{
		...
		$ReportDetails = $this->reports_model->getDetails('report', array('user_id' => $filtered_client_id, 'cost_id !=' => '', 'status' => 1, 'report_date >=' => $start_date, 'report_date <= ' => $end_date), '', array('orderby' => 'cost_id,report_date', 'disporder' => 'asc', 'custom_where' => '(report_type = "2" or( report_type="3" and ledger_type="2" ))'));
		// echo $this->db->last_query();
	...
	public function ajax_ledger_invoice()
	{
		...
		$ReportDetails = $this->reports_model->getDetails('report', array('user_id' => $filtered_client_id, 'status' => 1, 'report_date >=' => $start_date, 'report_date <= ' => $end_date), '', array('orderby' => 'cost_id', 'disporder' => 'asc', 'custom_where' => '(report_type="1" or report_type = "7" or report_type = "5" or report_type = "3" or report_type = "2")'));
		echo $this->db->last_query();



		$cost_txt = $cost_txt_sales = $cost_txt_expense = $cost_txt_actrev = $cost_txt_bankstatement = '';
		$excel_array = array();
		$excel_arr_sales = array();
		$excel_arr_actrev = array();
		$excel_arr_expense = array();
		$excel_arr_bstatement = array();
		$cost_txt = '<div id="header"><input type="hidden" class="inputh_year" value="' . $inputh_year . '"><p style="font-weight: bold;text-align: center;font-size:19px;margin: 12px 0px 0px;    text-transform: capitalize;">' . $filtered_client_name . '</p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">' . $ledger_title . ' </p><p style="text-align: center;font-size:13px;margin: 10px 0px;">From ' . $start_fulldate . ' - ' . $end_fulldate . '</p><br></div><div class="col-md-12">';
		...
		if ($ReportDetails) {
			...
			$excel_arr_bstatement[] = array($bankstatement_cost_head, '', '');
			$excel_arr_bstatement[] = array('', '', '');
			$excel_arr_bstatement[] = array('Date', 'RefNo', 'Details', 'Dr', 'Cr', 'Balance');
			...
			/**************** arul ->bank statement **************************/
			$cost_txt_bankstatement .= "<center><h3>" . $bankstatement_cost_head . "</small></h3></center><table class='table table-striped table-hover stl_costtbl'><thead><tr><th>Date</th><th>RefNo </th><th>Details</th><th  style='text-align:right;'>Dr</th><th  style='text-align:right;'>Cr</th><th  style='text-align:right;'>Balance</th></tr></thead><tbody>";

			$previous_child = '';
			$next_child = '';

			$total_amount = $salescost_id = 0;
			foreach ($ReportDetails as $ReportDetail) {
				$report_id = $ReportDetail->report_id;
				$cost_id = $ReportDetail->cost_id;
				$subcategory_id = $ReportDetail->subcategory_id;
				...
				if ($amount_type == 'sales') {
					...
					if ($report_type != 3 && $report_type != 2) {
						...
						$salescost_id = $cost_id;
						$actrevcost_id = $cost_id;
					...
					//Arul --> new code for bank statemnt 
					if ($report_type == 2) {

						$bstatement_total -= (float)$amount;
						$bstatement_balance = "R" . number_format((float)$bstatement_total, 2);
						$bstatement_cr_amt = "R" . number_format((float)$amount, 2);
						$bstatement_dr_amt = '';

						$cost_txt_bankstatement .= "<tr><td>" . $report_date . "</td><td>" . $ref_no . "</td><td>" . $description . "</td><td  style='text-align:right;'>" . $bstatement_dr_amt . "</td><td  style='text-align:right;'>" . $bstatement_cr_amt . "</td><td  style='text-align:right;'>" . $bstatement_balance . "</td></tr>";


						$excel_arr_bstatement[] = array($report_date, $ref_no, $description, '', $amount, $bstatement_total);
					}
					//Arul --> new code for bank statemnt end
					...
					//Arul --> new code for bank statemnt start
					if ($report_type == 2) {
						$bstatement_total += (float)$amount;
						$bstatement_balance = "R" . number_format((float)$bstatement_total, 2);
						$bstatement_dr_amt = "R" . number_format((float)$amount, 2);
						$bstatement_cr_amt = '';
				...
				// Arul -> if cost_id is in display the legder depands on the costcenter
				if ($cost_id != '' && $cost_id > 0) {
					/****   Arul ->If reporttype is journal and cost_is is accountreceiable ***/
					if ((($amount_type == 'sales' || $amount_type == 'expense') && ($report_type == 3 && $ledger_type == 1)) && ($cost_id == $actrev_cost_id)) {
					...
					/****   Arul ->If reporttype is journal and cost_id is sales ***/
					elseif ((($amount_type == 'sales' || $amount_type == 'expense') && ($report_type == 3 && $ledger_type == 1)) && ($cost_id == $sales_cost_id)) {
						if ($amount_type == 'expense') {
							$sales_total -= (float)$amount;
							$sales_balance = "R" . number_format((float)$sales_total, 2);
							$sales_cr_amt = "R" . number_format((float)$amount, 2);
					...
					/****   Arul ->If reporttype is journal and cost_id is in bank statemant ***/
					elseif ((($amount_type == 'sales' || $amount_type == 'expense') && ($report_type == 3 && $ledger_type == 1)) && ($cost_id == $bankstatement_cost_id)) {
						if ($amount_type == 'expense') {
							$bstatement_total -= (float)$amount;
							$bstatement_balance = "R" . number_format((float)$bstatement_total, 2);
							$bstatement_cr_amt = "R" . number_format((float)$amount, 2);
						...
						$cost_txt_bankstatement .= "<tr><td>" . $report_date . "</td><td>" . $ref_no . "</td><td>" . $description . "</td><td  style='text-align:right;'>" . $bstatement_dr_amt . "</td><td  style='text-align:right;'>" . $bstatement_cr_amt . "</td><td  style='text-align:right;'>" . $bstatement_balance . "</td></tr>";
						$excel_arr_bstatement[] = array($report_date, $ref_no, $description, $excel_dr_amt, $amount, $bstatement_total);
					}

					/****   Arul ->If reporttype is journal,expense and  cost_id is in expense ***/
					elseif ((($amount_type == 'sales' || $amount_type == 'expense') && ($report_type == 3 && $ledger_type == 1)) && ($cost_id == $expense_cost_id)) {
						if ($amount_type == 'expense') {
							$expense_total -= (float)$amount;
							$expense_balance = "R" . number_format((float)$expense_total, 2);
							$expense_cr_amt = "R" . number_format((float)$amount, 2);
					...
					/****   Arul ->If reporttype is journal,not in receiable, sales,payable ***/
					else {
						$excludecost = array($expense_cost_id, $actrev_cost_id, $sales_cost_id, $bankstatement_cost_id);
						if ((!in_array($cost_id, $excludecost) && ($report_type != 2 && $report_type != 3)) || ($report_type == 3 && $ledger_type == 1)) {
							if (($cost_id != '' && $cost_id > 0)) {
								...
								/****   Arul ->if amountype is expose display in CR,if amountype is sales display in DR reversr order fro return type 3  ***/
								if ($report_type == 3) {
									if ($amount_type == 'expense') {
										$total_amount -= (float)$amount;
										$balance = "R" . number_format((float)$total_amount, 2);
						...
						//Arul -> Display the bank statemnet with bank transactions
						else {
							if ((!in_array($cost_id, $excludecost) &&  $report_type != 2) && ($report_type == 3 && $ledger_type == 2)) {
								if (($cost_id != '' && $cost_id > 0)) {
									...
									$excel_arr_bstatement[] = array($report_date, $ref_no, $description, $excel_dr_amt, $excel_cr_amt, $bstatement_total);
								}
							}
						}
					}
			...
			$oformat_amount_bstatement = "R" . number_format((float)$bstatement_total, 2);
			...
			//Arul --> new code for bank statemnt end
			$cost_txt_bankstatement .= "</tbody><tfoot><tr><td colspan='6' style='text-align:right;'>" . $oformat_amount_bstatement . "</td></tr></tfoot></table>";
			...
			$excel_arr_bstatement[] = array('', '', '', '', '', '');
			$excel_arr_bstatement[] = array('', '', '', '', '', $bstatement_total);
			$excel_arr_bstatement[] = array('', '', '', '', '', '');
			$excel_arr_bstatement[] = array('', '', '', '', '', '');
		}
		//Arul --> new code for bank statemnt end
		// $excel_array = array_merge($excel_arr_actrev,$excel_arr_sales,$excel_arr_expense,$excel_array);
		$excel_array = array_merge($excel_arr_actrev, $excel_arr_sales, $excel_arr_expense, $excel_array, $excel_arr_bstatement);
		//Arul --> new code for bank statemnt end
	...
	public function ledger_pdf_invoice()
	{
		...
		//Arul-->added the report_type 2 for bank statement
		$ReportDetails = $this->reports_model->getDetails('report', array('user_id' => $filtered_client_id, 'status' => 1, 'report_date >=' => $start_date, 'report_date <= ' => $end_date), '', array('orderby' => 'cost_id', 'disporder' => 'asc', 'custom_where' => '(report_type="1" or report_type = "7" or report_type = "5"  or report_type = "3" or report_type = "2")'));
		// echo $this->db->last_query();
		...
		$cost_txt = $cost_txt_sales = $cost_txt_expense = $cost_txt_bstatement = $cost_txt_actrev = $cost_txt_other = '';

		$cost_txt = '<div id="header"><input type="hidden" class="inputh_year" value="' . $inputh_year . '"><p style="font-weight: bold;text-align: center;font-size:19px;margin: 12px 0px 0px;    text-transform: capitalize;">' . $filtered_client_name . '</p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">' . $ledger_title . ' </p><p style="text-align: center;font-size:13px;margin: 10px 0px;">From ' . $start_fulldate . ' - ' . $end_fulldate . '</p><br></div><div class="col-md-12">';

		$loop_start = $array_count = 0;
		$bstatement_total = 0;
		$sales_total = $actrev_total =  $balance = $expense_total = 0;
		$next_child = '';
		$previous_child = '';
		...
		if ($ReportDetails) {
			...
			$actrev_cost_head = $actrev_cost_name . "(" . $actrev_cost_link . ")";
			$expense_cost_head = $expense_cost_name . "(" . $expense_cost_link . ")";
			//Arul ->bankstatement title
			$bstatement_cost_head = $bankstatement_cost_name . "(" . $bankstatement_cost_link . ")";

			$cost_txt_sales .= "<center><h3>" . $sales_cost_head . "</small></h3></center><table class='table table-striped table-hover stl_costtbl'><thead><tr><th>Date</th><th>RefNo</th><th>Details</th><th  style='text-align:right;'>Dr</th><th  style='text-align:right;'>Cr</th><th  style='text-align:right;'>Balance</th></tr></thead><tbody>";
			$cost_txt_actrev .= "<center><h3>" . $actrev_cost_head . "</small></h3></center><table class='table table-striped table-hover stl_costtbl'><thead><tr><th>Date</th><th>RefNo</th><th>Details</th><th  style='text-align:right;'>Dr</th><th  style='text-align:right;'>Cr</th><th  style='text-align:right;'>Balance</th></tr></thead><tbody>";
			$cost_txt_expense .= "<center><h3>" . $expense_cost_head . "</small></h3></center><table class='table table-striped table-hover stl_costtbl'><thead><tr><th>Date</th><th>RefNo</th><th>Details</th><th  style='text-align:right;'>Dr</th><th  style='text-align:right;'>Cr</th><th  style='text-align:right;'>Balance</th></tr></thead><tbody>";
			$cost_txt_bstatement .= "<center><h3>" . $bstatement_cost_head . "</small></h3></center><table class='table table-striped table-hover stl_costtbl'><thead><tr><th>Date</th><th>RefNo</th><th>Details</th><th  style='text-align:right;'>Dr</th><th  style='text-align:right;'>Cr</th><th  style='text-align:right;'>Balance</th></tr></thead><tbody>";
			...
			foreach ($ReportDetails as $ReportDetail) {
				...
				if ($amount_type == 'sales') {
					...
					if ($report_type == 2) {
						...
						$cost_txt_bstatement .= "<tr><td>" . $report_date . "</td><td>" . $ref_no . "</td><td>" . $description . "</td><td  style='text-align:right;'>" . $bstatement_dr_amt . "</td><td  style='text-align:right;'>" . $bstatement_cr_amt . "</td><td  style='text-align:right;'>" . $bstatement_balance . "</td></tr>";
					}
					//Arul --> new code for bank statemnt end
						...
						//Arul -> Display the bank statemnet with bank transactions
							...
							if ((!in_array($cost_id, $excludecost) &&  $report_type != 2) && ($report_type == 3 && $ledger_type == 2)) {
								if (($cost_id != '' && $cost_id > 0)) {
									...
									$cost_txt_bstatement .= "<tr><td>" . $report_date . "</td><td>" . $ref_no . "</td><td>" . $description . $bstatement_total . "</td><td  style='text-align:right;'>" . $bstatement_dr_amt . "</td><td  style='text-align:right;'>" . $bstatement_cr_amt . "</td><td  style='text-align:right;'>" . $bstatement_balance . "</td></tr>";
			...
			$cost_txt_expense .= "</tbody><tfoot><tr><td colspan='6' style='text-align:right;'>" . $oformat_amount_expense . "</td></tr></tfoot></table>";
			$cost_txt_bstatement .= "</tbody><tfoot><tr><td colspan='6' style='text-align:right;'>" . $oformat_amount_bstatement . "</td></tr></tfoot></table>";
		...
		$cost_txt = $cost_txt . $cost_txt_actrev . $cost_txt_sales . $cost_txt_expense . $cost_txt_other . $cost_txt_bstatement;
		$pdf_fname = 'ledger_report.pdf';
		/********************* PDF start ********/
		$this->load->library('m_pdf');
		//$pdf_fname = 'rrrrrrrrrrrrrrrrrr.pdf';
	...
	/**************************** aruljothi editedcode***************/
	public function ledger_pdf_invoice_vijioldcode()
	{
		setlocale(LC_MONETARY, 'en_IN');
		$cbd_txt_pdf  = '';
		$cbd_txt = '';
		...
		$ReportDetails = $this->reports_model->getDetails('report', array('user_id' => $filtered_client_id, 'status' => 1, 'report_date >=' => $start_date, 'report_date <= ' => $end_date), '', array('orderby' => 'report_date', 'disporder' => 'asc', 'custom_where' => '(report_type="1" or report_type = "7" or report_type = "5")'));
		// echo $this->db->last_query();
	...
	public function ajax_financial_statement_invoice_pdf()
	{
				...
				if ($ReportDetails || $ReportDetails_journal) {
					...
					if ($ReportDetails) {
						foreach ($ReportDetails as $ReportDetail) {
							...
							if ($subcat_id_ar == SALES_COST_SUBCAT_ID || $subcat_id_ar == EXPENSE_COST_SUBCAT_ID) {
								$amount = -$amount;
							}
	...
	/*************** trial balance start *******************/
	public function trialbalance_creation($report_type = 1)
	{
		$client_id = $this->session->id;
		$start_month = '';
		$usertype = $this->session->usertype;
		...
		if ($report_type == '1') {
			$report_url = 'invoice_trialbal';
			$ledger_title = 'Invoicing Trial Balance';
			$ledger_menu = 'invoice_trialbal';
		} else {
			$report_url = 'banktrans_trialbal';
			$ledger_title = "Bank Transaction Trial Balance";
			$ledger_menu = 'banktrans_trialbal';
		}
		...
		// echo "<pre>";print_r($bank_stat_yr);echo "</pre>";
		$data = array(
			'view_file' => 'trial_balance_report',
			'current_menu' => $ledger_menu,
			'cusotm_field' => 'report',
			'site_title' => 'eShiro',
			'logo'		=> 'logo',
	...
	public function ajax_trial_balance_invoice()
	{

		setlocale(LC_MONETARY, 'en_IN');

		// $excel_array_final = $excel_array_asset = $excel_array_total_ast = $excel_array_libt = $excel_array_eqt = $excel_array_other = array();
		$excel_array = $excel_array_head = array();
		$final_ct = $asset_ct = $total_ast_ct = $array_libt_ct = $array_eqt_ct = $other_ct = 0;
		...
		if ($report_type == '1') {
			$report_url = 'invoice_trialbal';
			$ledger_title = 'Invoice Trial Balance';
			$ledger_menu = 'invoice_trialbal';
		} else {
			$report_url = 'banktrans_financial';
			$ledger_title = "Bank Transaction Financial";
		...
		$end_date = date('Y-m-t', strtotime($end_date));
		$end_fulldate = date('F d, Y', strtotime($end_date));
		// $ReportDetails = $this->reports_model->getDetails('report',array('user_id' => $filtered_client_id,'cost_id !=' => '','status' => 1,'report_type' => $report_type,'report_date >=' => $start_date,'report_date <= ' => $end_date),'',array('orderby' => 'cost_id,report_date','disporder' => 'asc'));
		// echo $this->db->last_query();
		...
		foreach ($subcat_id_loop as $sup_key  => $subcat_id_arr) {
			...
			foreach ($subcat_id_arr as $subcat_id_ar) {
				...
				if ($ReportDetails || $ReportDetails_journal) {
					...
					$oformat_amount = "R" . number_format((float)$total_amount, 2);
					$oformat_amount_xl = $total_amount;
					$cost_txthh = "</tbody><tfoot><tr><td colspan='2'>Total</td><td  style='text-align:right;'>" . $oformat_amount . "</td></tr></tfoot></table>";
	...
	public function ajax_trialbalance_invoice_pdf()
	{
	...
	public function fixed_asset_register($report_type = 1)
	{
		$client_id = $this->session->id;
		$usertype = $this->session->usertype;
		$client_id = $this->session->id;
		...
		if ($report_type == '1') {
			$menu_title = 'asset_register_invoice';
			$view_title = 'Invoice';
		} else {
			$menu_title = 'asset_register_bk';
			$view_title = 'Bank Statement';
		}
		// $asset_register_yr = $this->reports_model->get_asset_register_year($current_userid,$end_month,$report_type);

		$asset_data = $this->ajax_asset_register(array('client_id' => $current_userid, 'report_type' => $report_type));



		$data = array(
			'view_file' => 'fixed_asset_register',
			'current_menu' => $menu_title,
			'cusotm_field' => 'fixed_asset_register',
			'site_title' => 'eShiro',
			'logo'		=> 'logo',
			'title' => 'Report',
			'view_title' => $view_title,
			// 'CostDetails_arr' => $CostDetails_arr,
			// 'asset_register_yr' => $asset_register_yr,
	...
	public function ajax_asset_register($post_data = array())
	{
		$cost_dis_arr = $this->asset_register_function($post_data);
		// echo "<pre>";print_r($cost_dis_arr);echo "</pre>";
		$cost_dis = $cost_dis_arr['cost_txt'];

		/********************* EXCEL start ********/
		$this->excel->setActiveSheetIndex(0);
		//name the worksheet
		$this->excel->getActiveSheet()->setTitle('Asset Register');
		//set cell A1 content with some text
		$this->excel->getActiveSheet()->setCellValue('A1', $cost_dis_arr['filtered_client_name']);
		//$this->excel->getActiveSheet()->setCellValue('A2', 'Control Account');
		$this->excel->getActiveSheet()->setCellValue('A2', 'Asset Register');
		// $this->excel->getActiveSheet()->setCellValue('A3', 'From '.$cost_dis_arr['start_fulldate'].' - '.$cost_dis_arr['end_fulldate']);
		//$header = array('Date', 'File Name', 'File No', 'Transaction Type', 'DR', 'CR', 'Balance');

		$this->excel->getActiveSheet()->setCellValue('A5', 'S.No');
		$this->excel->getActiveSheet()->setCellValue('B5', 'Assets name	');
		// $this->excel->getActiveSheet()->setCellValue('C5', 'Report date	');
		$this->excel->getActiveSheet()->setCellValue('C5', 'Cost');
		$this->excel->getActiveSheet()->setCellValue('D5', 'Dep');
		$this->excel->getActiveSheet()->setCellValue('E5', 'Cum Dep	');
		$this->excel->getActiveSheet()->setCellValue('F5', 'NBV');
		//merge cell A1 until C1
		$this->excel->getActiveSheet()->mergeCells('A1:F1');
		$this->excel->getActiveSheet()->mergeCells('A2:F2');
		$this->excel->getActiveSheet()->mergeCells('A3:F3');
		...
		// $this->excel->getActiveSheet()->getStyle('A1')->getFont()->setSize(16);
		// $this->excel->getActiveSheet()->getStyle('A1')->getFill()->getStartColor()->setARGB('#333');
		for ($col = ord('D'); $col <= ord('F'); $col++) { //set column dimension $this->excel->getActiveSheet()->getColumnDimension(
		...
		foreach ($cost_dis_arr['excel_ttfoot'] as $val) {
			$this->excel->getActiveSheet()->getStyle('A' . $val . ':F' . $val)->getFont()->setBold(true);
			// $this->excel->getActiveSheet()->getStyle('A'.$val.':C'.$val)->getFont()->setSize(10);
			// $this->excel->getActiveSheet()->getStyle('C'.$val)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		}

		$this->excel->getActiveSheet()->fromArray($cost_dis_arr['excel_array'], null, 'A6');     // insert value to EXCEL
		...
		$objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
		$objWriter->save(FCPATH . 'uploads/asset_register.xls');
		/********************* EXCEL End ********/
		return $cost_dis;
		exit;
	}
	public function ajax_asset_register_pdf()
	{
		$cost_txt_arr = $this->asset_register_function($_POST);
		$cost_txt = $cost_txt_arr['cost_txt'];

		$pdf_fname = 'asset_register.pdf';

		/********************* PDF start ********/
		$this->load->library('m_pdf');
		//$pdf_fname = 'rrrrrrrrrrrrrrrrrr.pdf';

		$page_data['page_name']  	= 'Asset Register Report';
		$page_data['table_div'] = $cost_txt;
		//$page_data['report_results'] 	= $report_results;
		$this->load->view('invoice_ledger_pdf', $page_data);
		// Get output html
	...
	function dateDiffInDays($date1, $date2)
	{
		...
		public function asset_register_function($post_data = array())
	{
		$client_id = $post_data['client_id'];
		$current_date = date('Y-m-d');
		// $start_month = $post_data['start_month'];
		// $end_month = $post_data['end_month'];
		// $year = $post_data['year'];
		$report_type = $post_data['report_type'];
		$excel_array = $excel_ttfoot = array();
		$excel_arrcount = 6;
		// $inputh_year = $year;
		// $years=explode("-",$year);
		...
		$end_date = $year_end."-".$filtered_end_month."-01";
		$end_date = date('Y-m-t',strtotime($end_date));*/
		// $AssetDetails = $this->reports_model->getDetails('report',array('user_id' => $client_id,'status' => 1,'cost_id !=' => '','subcategory_id' => 7,'report_date >=' => $start_date,'report_date <= ' => $end_date));
		$AssetDetails = $this->reports_model->get_asset_register_data($client_id, $report_type);
		...
		// $asset_arr = '<div id="header"><input type="hidden" class="inputh_year" value="'.$inputh_year.'"><p style="font-weight: bold;text-align: center;font-size:19px;margin: 12px 0px 0px;    text-transform: capitalize;">'.$filtered_client_name.'</p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">Asset Register </p><p style="text-align: center;font-size:13px;margin: 10px 0px;">From '.$start_fulldate.' - '.$end_fulldate.'</p><br></div><div class="col-md-12">';
		$asset_arr = '<div id="header"><p style="font-weight: bold;text-align: center;font-size:19px;margin: 12px 0px 0px;    text-transform: capitalize;">' . $filtered_client_name . '</p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">Asset Register </p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">as at ' . date('F d, Y') . '</p><br></div><div class="col-md-12">';

		$asset_arr .= '<table class="table table-striped table-hover table-bordered1 stl_table "><thead><tr><td>S.no</td><td>Assets name</td><td>Acquisition Date</td><td>Cost</td><td>Dep</td><td>Cum Dep</td><td>NBV</td></tr></thead><tbody>';
		...
		if (!empty($AssetDetails)) {
			foreach ($AssetDetails as $AssetDetail) {
				...
				if ($old_cost_name != $cost_name) {
					...
					$asset_arr .= "<tr><th colspan='3'  style='text-align:center;'>Total " . $total_cost_name . "</th><th>R" . $oitem_total_cost . "</th><th>R" . $oitem_total_dep . "</th><th>R" . $oitem_total_cumdep . "</th><th>R" . $oitem_total_nbv . "</th></tr>";
					$excel_array[] = array("Total " . $total_cost_name, '', '', $item_total_cost, $oitem_total_dep, $oitem_total_cumdep, $oitem_total_nbv);
				...
				if($nbv_o > $AssetDetail->residual_value){
					$nbv = $amt - $cumdep;
				}else{
					$nbv = $AssetDetail->residual_value;
					$cumdep = $amt - $AssetDetail->residual_value;	
				}
				...
				$asset_arr .= '<tr><td>' . $i . '</td><td>' . $cost_name . '</td><td>' . $AssetDetail->report_date . '</td><td>R' . $oamt . ' </td><td>R' . $odepre . '</td><td>R' . $ocumdep . '</td><td>R' . $onbv . '</td></tr>';

				$excel_array[] = array($i, $cost_name, $AssetDetail->report_date, $amt, $odepre, $ocumdep, $onbv);
				$excel_arrcount++;
			}
			...
			$asset_arr .= "<tr><th colspan='3' style='text-align:center;'>Total " . $cost_name . "</th><th>R" . $oitem_total_cost . "</th><th>R" . $oitem_total_dep . "</th><th>R" . $oitem_total_cumdep . "</th><th>R" . $oitem_total_nbv . "</th></tr>";
			$excel_array[] = array("Total " . $cost_name, '', '', $item_total_cost, $oitem_total_dep, $oitem_total_cumdep, $oitem_total_nbv);
			$excel_ttfoot[] = $excel_arrcount;
			$excel_arrcount++;
			$excel_arrcount++;
			$excel_arrcount++;
			...
			$asset_arr .= "<tr><th colspan='3'  style='text-align:center;'>Total </th><th>R" . $ooverall_cost . "</th><th>R" . $ooverall_dep . "</th><th>R" . $ooverall_cumdep . "</th><th>R" . $ooverall_nbv . "</th></tr>";

			$excel_array[] = array();
			$excel_array[] = array();
			$excel_array[] = array("Total", '', '', $overall_cost, $ooverall_dep, $ooverall_cumdep, $ooverall_nbv);
			$excel_ttfoot[] = $excel_arrcount;
		}
		$asset_arr .= '</tbody></table></div>';

		return array('cost_txt' => $asset_arr, 'excel_array' => $excel_array, 'filtered_client_name' => $filtered_client_name, 'excel_ttfoot' => $excel_ttfoot);
	}
	/******************* asset register end ******************/
}