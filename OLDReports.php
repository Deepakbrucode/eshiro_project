	...
	public function ajax_cashbook(){
		...
		if(!empty($OpenBalDetails))
		{
			foreach ($OpenBalDetails as $OpenBalDetail) {
               ...
               $openbal_date = date('d-M-Y', strtotime($openbal_date));
				...
				$csvarray[$kk]['transaction_type']= '';
				$csvarray[$kk]['description']='Opening Balance';
				$csvarray[$kk]['fileno']='';
				$csvarray[$kk]['filename']= '';
				$csvarray[$kk]['dr']= $OpenBalDetail->amount;
				$csvarray[$kk]['cr']='';
		...
		if(!empty($ReceiptsDetails))
		{
			...
			foreach ($ReceiptsDetails as $ReceiptsDetail) {
				...
				$csvarray[$kk]['transaction_type']= $ReceiptsDetail->transaction_type;
				$csvarray[$kk]['description']= $ReceiptsDetail->details;
				$csvarray[$kk]['fileno']=$ReceiptsDetail->file_number;
				$csvarray[$kk]['filename']=$ReceiptsDetail->file_name;
				$csvarray[$kk]['dr']= $ReceiptsDetail->amount;
				$csvarray[$kk]['cr']='';
		...
		if(!empty($PaymentDetails))
		{
			...
			foreach ($PaymentDetails as $PaymentDetail) {
				...
				$csvarray[$kk]['transaction_type']= $PaymentDetail->transaction_type;
				$csvarray[$kk]['description']= $PaymentDetail->details;
				$csvarray[$kk]['fileno']=$PaymentDetail->file_number;
				$csvarray[$kk]['filename']=$PaymentDetail->file_name;
				$csvarray[$kk]['filename']=$PaymentDetail->file_name;
				$csvarray[$kk]['dr']= '';
				$csvarray[$kk]['cr']= $PaymentDetail->amount;
		...
		foreach ($csvarray as $data) {
			...
			if($cr_val !='' ) {
				/*$cr_amount = "R".money_format('%!i', $cr_val);
				$cr_amount_xl = money_format('%!i', $cr_val);*/
				$cr_amount = "R".number_format((float)$cr_val, 2);
				$cr_amount_xl = number_format((float)$cr_val, 2);
			} 
			else {$cr_amount ="";$cr_amount_xl = '';}
			if($dr_val !='' ) {
				/*$dr_amount = "R".money_format('%!i', $dr_val);
				$dr_amount_xl = money_format('%!i', $dr_val);*/
				$dr_amount = "R".number_format((float)$dr_val, 2);
				$dr_amount_xl = number_format((float)$dr_val, 2);
			} 
			else {$dr_amount ="";$dr_amount_xl='';}
			//$dr_amount = "R".money_format('%!i', $dr_val);
			/*$bal_amount = "R".money_format('%!i', $balance_total);
			$bal_amount_xl = money_format('%!i', $balance_total);*/
			$bal_amount = "R".number_format((float)$balance_total, 2);
			$bal_amount_xl = number_format((float)$balance_total, 2);

			$new_array = array( $data['date'], $data['description'], $data['fileno'], $data['filename'], $dr_amount,  $cr_amount, $bal_amount);
			$excel_array[] = array( $data['date'], $data['description'] , $data['fileno'], $data['filename'], $dr_amount_xl, $cr_amount_xl, $bal_amount_xl);
			$ajax_array[] = array( $data['date'], $data['description'] , $data['fileno'], $data['filename'], $dr_amount, $cr_amount, $bal_amount);
			$array_count++;
        ...
        // $this->excel->getActiveSheet()->getStyle('A1')->getFill()->getStartColor()->setARGB('#333');
       	for($col = ord('E'); $col <= ord('G'); $col++){ //set column dimension $this->excel->getActiveSheet()->getColumnDimension(
            ...
            $bal_amount = number_format((float)$balance_total, 2);
...
$financial_array = [];
$filtered_client_id= $this->session->filtered_client_id;
$filtered_start_month= $this->session->filtered_start_month;
$filtered_end_month= $this->session->filtered_end_month;
//$CashDetails = $this->reports_model->get_CashMonthYear($filtered_client_id, $filtered_start_month, $filtered_end_month);
$ReceiptsDetails = $this->reports_model->cashbook_lreceipt($filtered_client_id, $filtered_start_month, $filtered_end_month);
$PaymentDetails = $this->reports_model->cashbook_lpayment($filtered_client_id, $filtered_start_month, $filtered_end_month);
$OpenbalancesDetails = $this->reports_model->cashbook_lopenbal($filtered_client_id, $filtered_start_month, $filtered_end_month);
if($ReceiptsDetails)
{
	foreach ($ReceiptsDetails as $ReceiptsDetail) {
...
$full_details = [];
        ...
        //set cell A1 content with some text
        $this->excel->getActiveSheet()->setCellValue('A1', $filtered_client_name);
        $this->excel->getActiveSheet()->setCellValue('A2', 'CASHBOOK');
        ...
        $this->excel->getActiveSheet()->setCellValue('A7', 'TRUST CASHBOOK');
		...
		$cbd_txt = '<table style="width:100%"><thead><tr><td style="border:0px"><p style="font-weight: bold;text-align: center;font-size:19px;margin: 0px;">'.$filtered_client_name.'</p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">TRUST CASHBOOK </p><p style="text-align: center;font-size:13px;margin: 10px 0px;">From '.$start_fulldate.' - '.$end_fulldate.'</p><br></td></tr></thead><tbody><tr><td style="border:0px;">';

		$cbd_txt_pdf = '<div id="header"><p style="font-weight: bold;text-align: center;font-size:19px;margin: 0px;">'.$filtered_client_name.'</p><p style="text-align: center;font-size:13px;    margin: 10px 0px;font-weight: bold;">TRUST CASHBOOK </p><p style="text-align: center;font-size:13px;margin: 10px 0px;">From '.$start_fulldate.' - '.$end_fulldate.'</p><br></div><div class="col-md-12">';
        ...
        $this->excel->getActiveSheet()->getStyle('G'.$table_count)->getFont()->setBold(true);
				...
				$cbd_txt_pdf .= "<table class='table table-striped table-hover stl_table  cbd_table'><thead><tr><td>Date</td><td>Details</td><td>File No</td><td>File Name</td><td>Dr</td><td>Cr</td><td>Balance</td></tr></thead>";
		        ...
		        if(!empty($open_bal))
		        {
		            foreach($open_bal as $okey => $ovalue){
		               ...
		               /* if($oamount !='')
		                ...
		                $cbd_txt .= "<tr><td>".$odate_new."</td><td>Opening Balance</td><td></td><td></td><td>R".$balance_total_amt."</td><td></td><td>R".$balance_total_amt."</td></tr>";
			        ...
			        $this->excel->getActiveSheet()->setCellValue('E'.$table_count, $balance_total_amt);
			        $this->excel->getActiveSheet()->setCellValue('F'.$table_count, '');
		        ...
		        if(!empty($all_val))
		        {
		            foreach($all_val as $akey => $avalue){
                        ...
                        if($transaction_type == 'deposit'){$tran_txt = 'Deposit';$dr_amount = $oformat_amount; $dr_amount_xl = $oformat_amount_xl; $balance_total += $aamount;}
                        else if($transaction_type == 'rtd'){$tran_txt = 'RTD';$dr_amount = $oformat_amount;
                         $dr_amount_xl = $oformat_amount_xl;
                         $balance_total += $aamount; }
                        else if($transaction_type == 'credit_interest') {$tran_txt='Credit Interest';$dr_amount = $oformat_amount;
                        $dr_amount_xl = $oformat_amount_xl;
                         $balance_total += $aamount;}
                        else if($transaction_type == 'cost'){$tran_txt = 'Cost';$cr_amount = $oformat_amount;
                        $cr_amount_xl = $oformat_amount_xl;
                         $balance_total -= $aamount;}
                        else if($transaction_type == 'fee'){$tran_txt = 'Fee';$cr_amount = $oformat_amount; 
                        $cr_amount_xl = $oformat_amount_xl;
                        $balance_total -= $aamount;}
                        else if($transaction_type == 'refund'){$tran_txt = 'Refund';$cr_amount = $oformat_amount;
                        $cr_amount_xl = $oformat_amount_xl;
                        $balance_total -= $aamount;}
                        else {$tran_txt='Bank Charges';$cr_amount = $oformat_amount;
                        $cr_amount_xl = $oformat_amount_xl;
                        $balance_total -= $aamount;}
                        ...
                        if($details == ''){$details_txt = $tran_txt;}
                        else {$details_txt = $details;}


		                $cbd_txt .= "<tr><td>".$adate_new."</td><td>".$details_txt."</td><td>".$file_number."</td><td>".$file_name."</td><td>".$dr_amount."</td><td>".$cr_amount."</td><td>R".$balance_total_amt."</td></tr>";
		                $cbd_txt_pdf .= "<tr><td>".$adate_new."</td><td>".$details_txt."</td><td>".$file_number."</td><td>".$file_name."</td><td>".$dr_amount."</td><td>".$cr_amount."</td><td>R".$balance_total_amt."</td></tr>";


		                $this->excel->getActiveSheet()->setCellValue('A'.$table_count, $adate_new);
			        $this->excel->getActiveSheet()->setCellValue('B'.$table_count, $details_txt);
			        ...
			        $this->excel->getActiveSheet()->setCellValue('E'.$table_count, $dr_amount_xl);
			        $this->excel->getActiveSheet()->setCellValue('F'.$table_count, $cr_amount_xl);
			        $this->excel->getActiveSheet()->setCellValue('G'.$table_count, $balance_total_amt);
		            ...
		            $cbd_txt .= "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td style='font-weight:bold;'>R".$balance_total_amt."</td><tr>";
		            $cbd_txt_pdf .= "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td style='font-weight:bold;'>R".$balance_total_amt."</td><tr>";
		        $table_count++;
...
*/
	...
	public function client_ledger(){
		...
		if($userdata){
			$filtered_client_id= $this->session->filtered_client_id;
			$ClientlDetails = $this->reports_model->getClientDetails_file(array('clients.client_id' => $filtered_client_id, 'file.account_status' => 1));
			$data = array(
	...
	public function clientledger_list(){
		...
		//$end_monthName_date = date('t', $ts);
	/*	 
		$cstart_date = date('Y-'.$filtered_start_month.'-01');
		$end_fulldate = date('Y-'.$filtered_end_month.'-01');
		$cend_date = date('Y-m-t', strtotime($end_fulldate));
...
*/
		 ...
		 $current_month = date('m');
				...
				if($current_month < '03')
				{
				 ...
				 $cstart_date = date('Y-03-01',strtotime('-1 year'));
				 $a_date = date('Y-02-01');
				$cend_date = date("Y-m-t", strtotime($a_date));

				 //$endDate = date('Y-02-'.$end_monthName_date);
				 $endDate_start = date('Y-02-01');
				...
				else
				{
					 ...
					 $cstart_date = date('Y-03-01');
					//  $a_date = date('Y-02-01');
					//$endDate = date("Y-m-t", strtotime($a_date));
					 //$endDates = date('Y-02-01' ,strtotime('+1 year'));
					 
					 $endDate_start = date('Y-02-01' ,strtotime('+1 year'));
					 $cend_date = date("Y-m-t", strtotime($endDate_start));
				}
				$cstart_date = '2016-03-01';
				$cend_date = '2017-02-28';
				$endDate_start = '2017-02-01';
				$month_endDate = '2017-02-28';


$startDate_out = date('F, d Y',strtotime($cstart_date));
$endDate_out = date('F, d Y',strtotime($cend_date));
$date_txt = 'From '.$startDate_out.' - '.$endDate_out;
	...
	$table_count = 5;
       ...
       // $this->excel->getActiveSheet()->getStyle('A2')->getFont()->setBold(true);
		...
		$client_ledger_list_header = '<p style="font-weight: bold;text-align: center;font-size:18px;margin: 0px;">'.$client_name.'</p><p style="text-align: center;font-size:15px;margin: 10px 0px;">Client Ledger </p><p style="text-align: center;font-size:15px;margin: 10px 0px;">'.$date_txt.'</p>';
		...
		$FilelDetails = $this->reports_model->getClientDetails_file(array('clients.client_id' => $filtered_client_id));
			...
			if($FilelDetails)
			{
				foreach($FilelDetails as $filedetail)
				{
					...
					$client_ledgerlist = $this->reports_model->client_ledger_list($filtered_client_id,$file_id,$cstart_date,$cend_date);
					//echo $this->db->last_query();
					//echo "<pre>";print_r($client_ledgerlist);echo "</pre>";
					$client_ledger_list .= '<br><p style="font-weight: bold;text-align: center;font-size:15px;margin: 10px; 0px 0px;">'.$file_name.'</p><p style="font-weight: bold;text-align: center;font-size:15px;margin-top: 10px;">'.$file_number.'</p>';


					$this->excel->getActiveSheet()->setCellValue('A'.$table_count, $file_name);
					...
					$table_count++;
					 ...
					 $this->excel->getActiveSheet()->setCellValue('A'.$table_count, $file_number);
		...
		$this->excel->getActiveSheet()->getStyle('D'.$table_count)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			        $this->excel->getActiveSheet()->getStyle('E'.$table_count)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			        $this->excel->getActiveSheet()->getStyle('F'.$table_count)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
 	  			...
 	  			$balance_total = 0;
                       ...
                       // $balance_total_amt = "R".money_format('%!i', $balance_total) ;
                        $balance_total_amt = "R".number_format((float)$balance_total, 2);

						$client_ledger_list .="<tr><td>".$selected_date."</td><td>".$details."</td><td>".$file_number."</td><td>".$dr_amount."</td><td>".$cr_amount."</td><td>".$balance_total_amt."</td></tr>";
	  ...
	  $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
		...
		$data = array(
			...
			'client_ledger_list_header' => $client_ledger_list_header
		...
		if($userdata){
			...
			else
			{
				...
				if($current_month < '03')
				{
				 ...
				 $endDate = date("Y-m-t", strtotime('2017-02-01'));
				...
				else
				{
					 ...
					 $endDate = date('Y-02-01' ,strtotime('+1 year'));
			...
			$front_date = date('F d, Y',strtotime($startDate)).' to '.date('F d, Y',strtotime($endDate));
			...
			$ReceiptsDetails_open = $this->receipts_model->getReceiptsDetails(array('receipt_date >=' => $startDate ,'receipt_date <=' => $endDate, 'client_id' => $filtered_client_id),'sum(amount) as receipt_total,count(file_id)');
			$PaymentDetails_open = $this->payments_model->getPaymentDetails(array('payment_date >=' => $startDate ,'payment_date <=' => $endDate, 'client_id' => $filtered_client_id),'sum(amount) as payment_total,count(file_id)');
  ...
  /*********** CSV function start *************/
		...
		$array_count=6;
		 ...
		 foreach ($trial_balance as $key => $value) {
            ...
            $payment_total_amt = "R".number_format((float)$payment_total, 2);
            //$receipt_total_amt = "R".money_format('%!i', $receipt_total);
            $receipt_total_amt = "R".number_format((float)$receipt_total, 2);
           ...
           // $balance_amt = "R".money_format('%!i', $balance);
            $balance_amt = "R".number_format((float)$balance, 2);
        ...
        /* foreach ($open_balance as $key => $value) {
               ...
               $open_bal_amt = "R".number_format((float)$open_bal, 2);
               $total_balance_amt = "R".number_format((float)$total_balance, 2);
          ...
          $total_balance_amt = "R".number_format($total_balance,2);
                                                ...
                                                if($remaining == '-0.00'){$remaining = 10;}
	 ...
	 /*********** EXCEL function start *************/
        ...
        //$this->excel->getActiveSheet()->setCellValue('A3', 'File No')
         $this->excel->getActiveSheet()->setCellValue('A3',$front_date);
       ...
       // $this->excel->getActiveSheet()->mergeCells('A4:D4');
        ...
        //$open_bal_amt = "R".money_format('%!i', $open_bal);
        $open_bal_amt = "R".number_format((float)$Open_total, 2);
			...
			else
			{
				...
				else
				{
					...
					//$endDate = date("Y-m-t", strtotime($a_date));
					 ...
					 $endDate_start = date('Y-02-01' ,strtotime('+1 year'));
					 $endDate = date("Y-m-t", strtotime($endDate_start));
				}
				$startDate = '2016-03-01';
				$endDate = '2017-02-28';
				$endDate_start = '2017-02-01';
...
$startDate_out = date('F, d Y',strtotime($startDate));
$endDate_out = date('F, d Y',strtotime($endDate));
 ...
 /*********** PDF function start *************/
	...
	public function get_ClientLedger(){
		...
		if(!empty($FileOpenbalancesDetails))
		{
			foreach ($FileOpenbalancesDetails as $FileOpenbalancesDetail) {
				...
				$ledgerarray['open'][$kk]['description'] = 'opening_balance';
				$ledgerarray['open'][$kk]['ledger_type'] = 'opening_balance';

				$csvarray[$kk]['date']='';
				$csvarray[$kk]['date_str']= 1;
				$csvarray[$kk]['transaction_type']='Opening Balance';
				$csvarray[$kk]['description']= 'Opening Balance';
		...
		if(!empty($ReceiptsDetails))
		{
			...
			foreach ($ReceiptsDetails as $ReceiptsDetail) {
				...
				$ledgerarray['other'][$kk]['file_number'] = $ReceiptsDetail->file_number;
				$ledgerarray['other'][$kk]['transaction_type'] = $ReceiptsDetail->transaction_type;
				$ledgerarray['other'][$kk]['description'] = $ReceiptsDetail->details;
				$ledgerarray['other'][$kk]['ledger_type'] = 'receipt';
		...
		if(!empty($PaymentDetails))
		{
			...
			foreach ($PaymentDetails as $PaymentDetail) {
				...
				$ledgerarray['other'][$kk]['file_number'] = $PaymentDetail->file_number;
				$ledgerarray['other'][$kk]['transaction_type'] = $PaymentDetail->transaction_type;
				$ledgerarray['other'][$kk]['description'] = $PaymentDetail->details;
				$ledgerarray['other'][$kk]['ledger_type'] = 'payment';
	...
	//print_r($csvarray);
		...
		foreach ($csvarray as $data) {
			...
			$new_array = array( $data['date'],$data['description'], $data['fileno'], $dr_amount, $cr_amount, $bal_amount);
			$excel_array[] = array( $data['date'],$data['description'], $data['fileno'], $dr_amount_xl, $cr_amount_xl, $bal_amount_xl);
			$array_count++;
            fputcsv($csv_handler, $new_array);   // put array content into csv
        }
